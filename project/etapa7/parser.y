...
cmd : KW_IF expr KW_THEN cmd 	{ $$ = astCreate(AST_IF, 0,$2,$4,0,0,getLineNumber()); }
	...
	if(lineNumber != getLineNumber())
		{ 
			fprintf(stderr, "Syntax error on variable assignment at line %d\n",getLineNumber()); 
			errorFlag = 1; 
			lineNumber = getLineNumber();
		}
	}
	| TK_IDENTIFIER 'q' expr 'p' '=' error 	{ 
	if(lineNumber != getLineNumber())
		{ 
			fprintf(stderr, "Syntax error on vector assignment at line %d\n",getLineNumber()); 
			errorFlag = 1; 
			lineNumber = getLineNumber();
		}
	}
	...
	if(lineNumber != getLineNumber())
		{ 
			fprintf(stderr, "Syntax error on command at line %d\n",getLineNumber()); 
			errorFlag = 1; 
			lineNumber = getLineNumber();
		}
	}
...
exprError : expr '+' error  	{ 
	if(lineNumber != getLineNumber())
		{ 
			fprintf(stderr, "Syntax error on expression at line %d\n",getLineNumber()); 
			errorFlag = 1; 
			lineNumber = getLineNumber();
		}
	}
...
comma : id ',' comma  { $$ = astCreate(AST_COMMA, 0,$1,$3,0,0,getLineNumber()); }
	...
	if(lineNumber != getLineNumber())
		{ 
			fprintf(stderr, "Syntax error on comma separation at line %d\n",getLineNumber()); 
			errorFlag = 1; 
			lineNumber = getLineNumber();
		}
	}
...
id :  LIT_INTEGER				{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0,getLineNumber()); }
	...
	| TK_IDENTIFIER				{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0,getLineNumber()); }
	| TK_IDENTIFIER 'q' expr 'p'{ $$ = astCreate(AST_SYMBOL_VEC, $1,$3,0,0,0,getLineNumber()); }
	| TK_IDENTIFIER 'q' error 'p'{ fprintf(stderr, "Syntax error on vector at line %d\n",getLineNumber()); errorFlag = 1;}
	;
