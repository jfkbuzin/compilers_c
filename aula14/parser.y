...
expr : LIT_INTEGER				{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0); }
	...
	| TK_IDENTIFIER 'd' comma 'b'{ $$ = astCreate(AST_SYMBOL, $1,$3,0,0,0); }
	| TK_IDENTIFIER 'q' expr 'p'{ $$ = astCreate(AST_SYMBOL_VEC, $1,$3,0,0,0); }
	;


comma : id ',' comma  { $$ = astCreate(AST_COMMA, 0,$1,$3,0,0); }
	| id				{ $$ = $1; }
	;

id :  LIT_INTEGER				{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0); }
	| LIT_FLOAT					{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0); }
	| LIT_CHAR					{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0); }
	| LIT_STRING				{ $$ = astCreate(AST_SYMBOL_STRING, $1,0,0,0,0); }	
	| TK_IDENTIFIER				{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0); }
...
esc : TK_IDENTIFIER				{ $$ = astCreate(AST_SYMBOL, $1,0,0,0,0); }
	;
